...
public class GravityController_Touch : MonoBehaviour
{
    ...
    void ApplyCustomGravity()
    {
        Vector2 gravityVector =
            gravityDirection == GravityDirection.Right ? Vector2.right : Vector2.left;

        rb.AddForce(gravityVector * gravityStrength, ForceMode2D.Force);
    }
    ...
    void Jump()
    {
        ...
        Vector2 jumpDir =
            gravityDirection == GravityDirection.Right ? Vector2.left : Vector2.right;
    ...
    void TouchOrSwipe(Vector2 delta)
    {
        ...
        {
            SetGravityDirection(
                delta.x > 0 ? GravityDirection.Right : GravityDirection.Left
            );
            return;
        }
    ...
    {
        if (!controlsEnabled) return;
        if (gravityDirection == dir) return;
